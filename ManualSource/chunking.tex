...
\begin{figure}[!h]
	...
	\insertcaption{A close-up of a trace showing differences between a working memory trace (left) and a behavior trace (right).  The working memory trace only contains the literal values of the WMEs that matched.  The behavior trace, on the other hand, contains variables and various constraints on the values those variables can hold.}[A comparison of a working memory trace and a behavior trace]
...
\begin{figure}[!h]
	...
	\insertcaption{A visualization of the behavior trace of a chunk learned by the arithmetic agent.  Each box represents a rule that fired in the substate.  Arrows show dependencies between rules that create working memory elements and conditions that test those working memory elements.}[A visualization of a behavior trace]
	\label{fig:chunking-trace}
\end{figure}

%sjj -- not sure how good this paragraph is, but it's my understanding:
...
\begin{figure}[!ht]
	...
	\insertcaption{A behavior trace of two simple rules that matched in a substate}
...
\begin{figure}[!h]
	...
	\insertcaption{A behavior trace incorporating identity analysis}
...
\begin{figure}
	\centering
	\includegraphics[width=.8\textwidth]{chunking-ebbs-components.png}%fixme todo bugbug need attribution to Mazin's thesis.
	\insertcaption{Note that the two rows on the bottom indicate when each component occurs during Soar's processing. Bold components are new or significantly modified in EBBS.}[The nine components of explanation-based behavior summarization]
	\label{fig:chunking-ebbs-components}
\end{figure}

\begin{enumerate} %Mostly from page 62 in Mazin's thesis.

	\item \itextbf{Operator Selection Knowledge Analysis} \newline
	...
	\item \itextbf{Constraint Tracking} \newline
	This component stores every value or relational constraint (e.g. \soar{<> <x>}, \soar{>= 3.14}, \soar{<< disjunction of constants >>}) placed on variables.
	
	\item \itextbf{Collect Full Set of Inferences} \newline
	...
	\item \itextbf{Dependency Analysis} \newline
	...
	\item \itextbf{Identity Graph Manipulation} \newline
	...
	\item \itextbf{Constraint Collection} \newline
...
Constraints are stored relative to identities used in an instantiation. If a rule tests that \soar{<x> < 7}, EBBS would record on identity node 1 associated with \soar{<x>} that identity node 1 must have a value less than 7. Similarly, if another condition imposed the constraint \soar{<> <y>} on \soar{<x>} and \soar{<y>} was mapped to identity node 2, then EBBS would record on identity node 1 that it cannot have the same value as identity node 2.


% ----------------------------------------------------------------------------
\section{What EBBS Does During the Learning Episode}
...
\subsubsection*{The Identity Graph} %add stuff from earlier discussion of identity here and make the previous stuff less complicated.
...
\begin{itemize}
	\item stores constraints on the value a single identity, for example \soar{>= 0}, \soar{< 23}.
	\item stores relational constraints between two identities, for example \soar{> <min>},  \soar{< <max>} or  \soar{<> <other>}.
	\item stores all of these constraints based on the underlying identities, not the variables used.  For example, if a variable \soar{<foo>} had the constraint \soar{<> <other>}, EBBS would record that the variables that share the identity of \soar{<foo>} cannot have the same value as variables that share the identity of \soar{<other>}.
\end{itemize}
...
\subsubsection{Rule Validation and Repair}
...
Justifications use all the components described in the previous sections and are even affected by the current chunk settings.\footnote{
...
EBBS will incorporate relevant operator selection knowledge if you enable the chunk setting \soar{add-osk}, which is off by default. (See Section \ref{chunk-add-osk}.)
...
\begin{figure}[!ht]
	...
	\includegraphics[width=\textwidth]{chunking-trace-identity.png}
	%	}
	\insertcaption{A colored visualization of a behavior trace}
