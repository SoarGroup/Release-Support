...
For example, if LTI \soar{{@}5} had augmentations \soar{\^{}A do \^{}B re \^{}C mi}, and a \soar{store} command used short-term identifier \soar{L35} which was linked to \soar{{@}5} but had only the augmentation \soar{\^{}D fa}, the LTI \soar{{@}5} would be changed to only have \soar{\^{}D fa}.

\subsection{Store-new command}

The \soar{\^{}store-new} command structure is just like the \soar{\^{}store} command, except that smem will always store the given memory as an entirely new structure, regardless of whether the given STI was linked to an existing LTI or not. Any STIs that don't already have links will get linked to the newly created LTIs. But if a stored STI was already linked to some LTI, Soar will not re-link it to the newly created LTI. 

If this behavior is not desired, the agent can add a \soar{\^{}link-to-new-LTM yes} augmentation to override this behavior. One use for this setting is to allow chunking to backtrace through a stored memory in a manner that will be consistent with a later state of memory when the newly stored LTI is retrieved again.
